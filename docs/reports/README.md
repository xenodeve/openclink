...
- [2026-08-13 — CLI enforcement capability: every clink client can block a tool call](2026-08-13-cli-enforcement-capability.md)
  — **overturns the Q2 verdict of the Phase 0 spike.** That spike measured from `--help` and concluded only the Claude Code harness had a blocking pre-tool hook; probing the shipped binaries instead found `codex` (11 hook events, three pre-action, schema extracted from the binary), `agy` and `cursor-agent` all able to deny a tool call. Also records the stronger lever, **since verified end to end**: `--tools` on Claude Code *removes* a built-in from context while MCP tools sit outside that layer, so a PAL-provided edit tool can be the only writing instrument a worker has — a worker spawned with `--tools ""` against a live MCP server enumerated that one MCP tool as its entire toolset and could not read a file. The same probe enlarged #89: removing built-ins also removes `Read`, `Grep`, `Bash` and `Skill`, so the usable recipe is a read-only allowlist plus PAL's own instrument, and #88's skill floor has to arrive as prompt text. #16's gate rests on the superseded premise.
