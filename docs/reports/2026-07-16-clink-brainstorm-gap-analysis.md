...
| 4 | **All CLIs are told "You are operating through the Gemini CLI agent"** | ✅ `tools/clink.py:487` `_agent_capabilities_guidance()` hardcodes the Gemini string, returned verbatim for codex/claude/antigravity too | Gives every delegated agent a false identity/capability set. | S |
