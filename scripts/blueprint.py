...
def main() -> int:
    ...
    two_cycles = sorted(
        {tuple(sorted((m, dep))) for m, info in mods.items() for dep in info["imports"] if m in mods.get(dep, {}).get("imports", [])}
    )
    ...
    w("")
    w("# PAL MCP — generated structure")
    w("")
    w(f"**{len(mods)} production modules, {total_lines:,} lines.** "
    ...
    w("")
    if violations:
        w(f"**{len(violations)} edges cross a layer boundary backwards.** Each is a place where a change "
    ...
    for info in sorted(mods.values(), key=lambda i: (-len(i["imported_by"]), -i["lines"]))[:20]:
        w(f"| `{info['module']}` | {info['layer']} | {info['lines']:,} | {len(info['imported_by'])} | {info['test_files_mentioning']} |")
    w("")
    w("## Modules no test mentions")
    w("")
    silent = sorted([i for i in mods.values() if i["test_files_mentioning"] == 0], key=lambda i: -i["lines"])
    if silent:
        w(f"**{len(silent)} of {len(mods)} modules ({sum(i['lines'] for i in silent):,} lines) are named by no test file.** "
    ...
    for info in sorted(mods.values(), key=lambda i: (LAYER_RANK[i["layer"]], i["module"])):
        w(f"| `{info['module']}` | {info['layer']} | {info['lines']:,} | {len(info['classes'])} | "
    ...
    with open(dest, "w", encoding="utf-8", newline="\n") as fh:
        fh.write("\n".join(out))
    print(f"wrote {rel(dest)}  ({len(mods)} modules, {total_lines:,} lines, "
