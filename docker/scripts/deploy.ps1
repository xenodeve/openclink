...
Write-ColorText "=== Deploying OpenClink ===" -Color Green
...
function Wait-ForHealth {
    ...
    while ($attempt -le $MaxAttempts) {
        try {
            # Get container ID for openclink service
            $containerId = docker-compose ps -q openclink
...
# Start the services
Write-ColorText "Starting OpenClink..." -Color Green
try {
    docker-compose up -d
    if ($LASTEXITCODE -ne 0) {
...
Write-ColorText "✓ OpenClink deployed successfully" -Color Green
...
Write-ColorText "docker-compose logs -f openclink" -Color Green
...
Write-ColorText "docker-compose restart openclink" -Color Green
