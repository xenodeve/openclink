...
class TestDockerClaudeDesktopIntegration:
    ...
    def test_mcp_config_docker_run_format(self):
        ...
        assert "openclink" in config["mcpServers"]
        assert config["mcpServers"]["openclink"]["command"] == "docker"

        args = config["mcpServers"]["openclink"]["args"]
        assert "run" in args
        assert "--rm" in args
        assert "-i" in args
        ...
        config = {
            "mcpServers": {
                "openclink": {
                    ...
                    "args": ["-f", "/path/to/docker-compose.yml", "run", "--rm", "openclink"],
                }
            }
        }

        # Validate configuration structure
        assert config["mcpServers"]["openclink"]["command"] == "docker-compose"

        args = config["mcpServers"]["openclink"]["args"]
        assert "-f" in args
        assert "run" in args
        assert "--rm" in args
        assert "openclink" in args
    ...
    def test_windows_path_format(self):
        ...
        args = windows_config["mcpServers"]["openclink"]["args"]
    ...
    def test_mcp_config_validation(self):
        ...
        # Valid configuration
        valid_config = {
            "mcpServers": {"openclink": {"command": "docker", "args": ["run", "--rm", "-i", "openclink:latest"]}}
        }
    ...
    def test_docker_image_reference(self):
        ...
        configs = [
            {"image": "openclink:latest"},
            {"image": "openclink:v1.0.0"},
            {"image": "registry/openclink:latest"},
        ]
    ...
    def temp_mcp_config(self):
        ...
        config = {
            "mcpServers": {
                "openclink": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--env-file", "/tmp/.env", "openclink:latest"],
                }
            }
        }
...
class TestDockerMCPErrorHandling:
    ...
    def test_missing_docker_image_handling(self):
        ...
        nonexistent_config = {
            "mcpServers": {"openclink": {"command": "docker", "args": ["run", "--rm", "-i", "nonexistent:latest"]}}
        }

        # Configuration should be valid even if image doesn't exist
        assert "openclink" in nonexistent_config["mcpServers"]
    ...
    def test_invalid_env_file_path(self):
        ...
        config_with_invalid_env = {
            "mcpServers": {
                "openclink": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--env-file", "/nonexistent/.env", "openclink:latest"],
                }
            }
        }

        # Configuration structure should still be valid
        args = config_with_invalid_env["mcpServers"]["openclink"]["args"]
        assert "--env-file" in args
    ...
    def test_resource_limit_configurations(self):
        ...
        config_with_limits = {
            "mcpServers": {
                "openclink": {
                    "command": "docker",
                    "args": ["run", "--rm", "-i", "--memory=512m", "--cpus=1.0", "openclink:latest"],
                }
            }
        }

        args = config_with_limits["mcpServers"]["openclink"]["args"]
