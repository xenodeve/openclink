...
description: "bare clink commands are discovery-resolved to absolute paths; ~/.openclink/cli_clients overrides (the pre-rename ~/.pal/cli_clients is still read too); claude-9arm ships active"
metadata:
  type: reference
---
...
CLI installed under a user-profile dir still works when the editor launches OpenClink with a minimal PATH.
...
template. For a custom path/gateway, drop an override in **`~/.openclink/cli_clients/*.json`** — the
registry reads it last (overrides the bundled config), it survives reinstalls, it's shared by every
OpenClink install on the machine, and the pre-rename `~/.pal/cli_clients/` is still read too (see
[[openclink-two-installs-and-config-cache]]).

**How to apply:** to add a machine-specific client/path, prefer `~/.openclink/cli_clients/` over editing
